...
describe CloudQuote::Api::Client do
  before do
    CloudQuote::Api.config.reset!
    CloudQuote::Api.logger.reset!
  end
  context 'with defaults' do
    ...
    context '#initialize' do
      ...
      it 'includes default http configuration' do
        expect(client.user_agent).to eq "CloudQuote Ruby Client/#{CloudQuote::VERSION}"
      end
      it 'sets user-agent' do
        expect(client.user_agent).to eq CloudQuote::Api::Config::Client.user_agent
        expect(client.user_agent).to include CloudQuote::VERSION
      end
      ...
      CloudQuote::Api::Config::Client::ATTRIBUTES.each do |key|
        it "sets #{key}" do
          expect(client.send(key)).to eq CloudQuote::Api::Config::Client.send(key)
        end
      end
    end
  end
    ...
    context '#initialize' do
      CloudQuote::Api::Config::Client::ATTRIBUTES.each do |key|
        context key.to_s do
          ...
          it "sets #{key}" do
            expect(client.send(key)).to_not eq CloudQuote::Api::Config::Client.send(key)
  ...
  context 'global config' do
    ...
    context 'logger option' do
      ...
      context 'when assigning an instance' do
        context '#initialize' do
          context 'when directly assigning `logger`' do
            before { CloudQuote::Api.logger = logger }
          ...
          context 'when assigning through `configure.logger`' do
            it 'sets the logger' do
              CloudQuote::Api.configure.logger = logger
              expect(client.logger.instance).to eq(logger)
            end
          end
      ...
      context 'when assigning a configuration' do
        ...
        before do
          CloudQuote::Api.logger do |log_config|
    ...
    context 'when resetting/changing configuration' do
      before do
        CloudQuote::Api.configure { |config| config.user_agent = 'custom/user-agent' }
      end

      it 'does not reset the client' do
        expect { CloudQuote::Api.config.reset! }.not_to change(client, :user_agent).from('custom/user-agent')
      end

      it 'effects the next client' do
        pre_config_client = described_class.new
        CloudQuote::Api.configure { |config| config.user_agent = 'custom/user-agent-2' }
    ...
    context 'without a token' do
      ...
      it 'results in an API key error', vcr: { cassette_name: 'iex/client/access_denied' } do
        ...
        end.to raise_error CloudQuote::Errors::PermissionDeniedError, /The API key provided is not valid./
      end
    end
  end
end
