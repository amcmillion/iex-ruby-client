...
describe IEX::Resources::Dividends do
  ...
  context 'known symbol' do
    context 'with defaults', vcr: { cassette_name: 'iex/dividends/msft' } do
      subject do
        iex_client.dividends('MSFT', '6m')
      end
    ...
    context 'without range', vcr: { cassette_name: 'iex/dividends/msft_default_range' } do
      subject do
        iex_client.dividends('MSFT')
      end
      it 'retrieves dividends when no range is passed' do
        expect(subject.size).to eq 1
      end
    end
    context 'invalid range', vcr: { cassette_name: 'iex/dividends/msft_invalid_range' } do
      subject do
        iex_client.dividends('MSFT', 'INVALID')
      end
      let(:dividends) { subject.first }
    ...
    context 'with range', vcr: { cassette_name: 'iex/dividends/msft_1y' } do
      subject do
        iex_client.dividends('MSFT', '1y')
      end
  ...
  context 'invalid symbol', vcr: { cassette_name: 'iex/dividends/invalid' } do
    subject do
      iex_client.dividends('INVALID', 'INVALID')
    end
