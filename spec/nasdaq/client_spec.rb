...
describe Nasdaq::Api::Client do
  before do
    Nasdaq::Api.config.reset!
    Nasdaq::Api.logger.reset!
  end
  context 'with defaults' do
    ...
    context '#initialize' do
      ...
      it 'includes default http configuration' do
        expect(client.user_agent).to eq "NASDAQ Ruby Client/#{Nasdaq::VERSION}"
      end
      it 'sets user-agent' do
        expect(client.user_agent).to eq Nasdaq::Api::Config::Client.user_agent
        expect(client.user_agent).to include Nasdaq::VERSION
      end
      ...
      Nasdaq::Api::Config::Client::ATTRIBUTES.each do |key|
        it "sets #{key}" do
          expect(client.send(key)).to eq Nasdaq::Api::Config::Client.send(key)
        end
      end
    end
  end
    ...
    context '#initialize' do
      Nasdaq::Api::Config::Client::ATTRIBUTES.each do |key|
        context key.to_s do
          ...
          it "sets #{key}" do
            expect(client.send(key)).to_not eq Nasdaq::Api::Config::Client.send(key)
  ...
  context 'global config' do
    ...
    context 'logger option' do
      ...
      context 'when assigning an instance' do
        context '#initialize' do
          context 'when directly assigning `logger`' do
            before { Nasdaq::Api.logger = logger }
          ...
          context 'when assigning through `configure.logger`' do
            it 'sets the logger' do
              Nasdaq::Api.configure.logger = logger
              expect(client.logger.instance).to eq(logger)
            end
          end
      ...
      context 'when assigning a configuration' do
        ...
        before do
          Nasdaq::Api.logger do |log_config|
    ...
    context 'when resetting/changing configuration' do
      before do
        Nasdaq::Api.configure { |config| config.user_agent = 'custom/user-agent' }
      end

      it 'does not reset the client' do
        expect { Nasdaq::Api.config.reset! }.not_to change(client, :user_agent).from('custom/user-agent')
      end

      it 'effects the next client' do
        pre_config_client = described_class.new
        Nasdaq::Api.configure { |config| config.user_agent = 'custom/user-agent-2' }
